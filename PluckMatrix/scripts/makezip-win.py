...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\PluckMatrix Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\PluckMatrix Demo Installer.exe"
    ...
    files = [
      projectpath + "\\build-win\\PluckMatrix.vst3\\Contents\\x86_64-win\\PluckMatrix.vst3",
      projectpath + "\\build-win\\PluckMatrix_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\PluckMatrix-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\PluckMatrix-app_x64.pdb"  
  ]
